    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('posts.index')" :active="request()->routeIs('posts.*')">
                <i class="fas fa-blog mr-3 text-primary-500"></i>
                {{ __('Blog') }}
            </x-responsive-nav-link>
        </div>

        <!-- Responsive Settings Options - Ultra Compacto -->
        <div class="pt-2 pb-1 border-t border-gray-200">
            <div class="px-3 py-1">
                <div class="flex items-center justify-between">
                    <div class="flex-1 min-w-0">
                        <div class="text-xs text-gray-600 truncate">{{ Auth::user()->name }}</div>
                        <div class="text-xs text-gray-400 truncate">{{ Auth::user()->email }}</div>
                    </div>
                    <div class="flex items-center space-x-1 ml-2">
                        <a href="{{ route('profile.edit') }}" class="p-1.5 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded text-xs">
                            <i class="fas fa-user-edit"></i>
                        </a>
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded text-xs">
